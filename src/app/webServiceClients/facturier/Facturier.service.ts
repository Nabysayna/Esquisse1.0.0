...
@Injectable()
export class FacturierServiceWeb {


  private servicePort:string = 'http://51.254.200.129' ;
  private servicePath:string = '/backendprod/EsquisseBackEnd/web/app.php/invest/facturier?wsdl' ;
  private targetNamespace:string = 'urn:facturierwsdl' ;
  ...
  public paimentsde(montant : number, reference_client : number,reference_facture :number ,service :string) : Promise<any> {
      ...
      var method:string = 'reglementsde';
      var parameters:{}[] = [];

      var reEspParams = {token:this.token, reference_client:reference_client,reference_facture:reference_facture, service:service} ;
      var params:{}[] = [] ; 
      params["params"] = reEspParams ;
      parameters['reglementsde xmlns="urn:facturierwsdl#"'] = params ;

      return new Promise( (resolve, reject) => {
        this.soapService.post(method, parameters, 'reglementsdeResponse').then(response=>{
            let rep=JSON.parse(response["reglementsdeResponse"]["return"].$);
        
            console.log("Wizall nena : "+rep );
            resolve(rep) ;
        }) ;
      });
  }
  ...
  public detailreglementsde(reference_client:number):Promise<any>{
      let method:string='detailreglementsde';
      let parameters:{}[]=[];
      let reEspParams={token:this.token,reference_client:reference_client};
      let params:{}[]=[];
      params["params"]=reEspParams;

      parameters['detailreglementsde xmlns="urn:facturierwsdl#"']=params;

      return new Promise((resolve,reject) =>{       
        this.soapService.post(method,parameters,'detailreglementsdeResponse').then(response =>{
            let rep=response["detailreglementsdeResponse"]["return"].$;
